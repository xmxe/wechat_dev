...
public class PermissionController {
	...
	public DataGridView loadPermissionManagerLeftTreeJson(PermissionVo permissionVo) {
		...
		List<Permission> list = this.xinyePermissionService.list(queryWrapper);
	...
	public DataGridView loadAllPermission(PermissionVo permissionVo) {
		...
		this.xinyePermissionService.page(page, queryWrapper);
	...
	public Map<String, Object> loadPermissionMaxOrderNum() {
		...
		List<Permission> list = this.xinyePermissionService.page(page, queryWrapper).getRecords();
		if (list.size() > 0) {
	...
	public ResultObj addPermission(PermissionVo permissionVo) {
		try {
			...
			this.xinyePermissionService.save(permissionVo);
	...
	public ResultObj updatePermission(PermissionVo permissionVo) {
		try {
			this.xinyePermissionService.updateById(permissionVo);
	...
	public ResultObj deletePermission(PermissionVo permissionVo) {
		try {
			this.xinyePermissionService.removeById(permissionVo.getId());
