...
public class RoleController {
	...
	@Autowired
	private RoleService xinyeRoleService;
	
	@Autowired
	private PermissionService xinyePermissionService;
	...
	public DataGridView loadAllRole(RoleVo roleVo) {
		...
		this.xinyeRoleService.page(page, queryWrapper);
	...
	public ResultObj addRole(RoleVo roleVo) {
		try {
			...
			this.xinyeRoleService.save(roleVo);
	...
	public ResultObj updateRole(RoleVo roleVo) {
		try {
			this.xinyeRoleService.updateById(roleVo);
	...
	public ResultObj deleteRole(Integer id) {
		try {
			this.xinyeRoleService.removeById(id);
	...
	public DataGridView initPermissionByRoleId(Integer roleId) {
		...
		List<Permission> allPermissions = xinyePermissionService.list(queryWrapper);
		/**
		 * 1,根据角色ID查询当前角色拥有的所有的权限或菜单ID
		 * 2,根据查询出来的菜单ID查询权限和菜单数据
		 */
		List<Integer> currentRolePermissions=this.xinyeRoleService.queryRolePermissionIdsByRid(roleId);
		List<Permission> carrentPermissions=null;
		if(currentRolePermissions.size()>0) { //如果有ID就去查询
			queryWrapper.in("id", currentRolePermissions);		
			carrentPermissions = xinyePermissionService.list(queryWrapper);
		}else {
	...
	public ResultObj saveRolePermission(Integer rid,Integer[] ids) {
		try {
			this.xinyeRoleService.saveRolePermission(rid,ids);
